...
const { Message, UserKeys } = require('./persistence_user');
...
class KeyManager {
  ...
  async savePrivateKeys(dbNewUserKeys){
    try {
      const keys = new UserKeys(dbNewUserKeys);
      await keys.save();
  ...
  async uploadPublicKeys(dbNewPublicKeys){
    // dbNewPublicKeys: JSON de llaves
    try {
      // const keys = new UserKeys(dbNewPublicKeys);
      // await keys.save();
      axios.post("/api/v1/savekeys", dbNewPublicKeys)
        .then((response) => {
          // setMessages(response.data);
          console.log("Respuesta del servidor:", response.status);
          console.log("Usuario ",dbNewPublicKeys.username," enviado al servidor")
        })
        .catch((error) => {
          console.error("Error al publicar las llaves:", error);
        });
        
    } catch (err) {
      console.error("Error al publicar las llaves del usuario:", err);
    }
  }
  ...
  async create_user(){
    ...
    let privateKeys = {
      userId:userId,
      username:username,
      privateIdentityKey:this.identityKey.privateKey,
      privateSignedPrekey:this.signedPrekey.privateKey,
    ...
    this.uploadPublicKeys(publicKeys);
  }
